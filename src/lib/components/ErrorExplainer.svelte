...
<div class="mt-10 h-full max-w-[calc(100vw-40px)] w-full sm:mt-30vh lg:max-w-xl sm:max-w-lg sm:w-3/4">
  <div in:scale|global={{ start: 0.9, easing: cubicOut }} out:scale|global={{ start: 0.95, easing: cubicIn }} use:draggable={{ applyUserSelectHack: false, cancel: ref, bounds: { left: 20, top: 20, right: 20, bottom: 20 } }} class="max-h-[calc(100vh-40px)] cursor-grab rounded bg-neutral-8/60 ring-0.9 ring-white/20 backdrop-blur-lg xl:max-w-3xl lg:ring-1.3 sm:ring-1.1" class:!pb-0={output}>
    <div class="h-full w-full flex flex-col gap-3 bg-neutral-9/40 p-3 sm:gap-4 lg:p-4 sm:p-3.5">
